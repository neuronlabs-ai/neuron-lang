...
\begin{itemize}[itemsep=2pt]
  \item \textbf{Claim 1}: NEURON's type checker rejects programs that contain temporal leaks, causal mode confusion, and unguarded uncertainty access, as defined by our typing rules. \emph{Boundary}: NEURON enforces consistency of causal reasoning within a declared model; it does not verify that the declared model is correct. The core temporal-causal calculus ($\lambda_{\text{neuron}}$) is mathematically formalized with proofs of Progress, Preservation, and Temporal Non-Interference (\S\ref{sec:formal-semantics}); interactive theorem proving in Coq/Lean remains ongoing work.
  
  \item \textbf{Claim 2}: The compiler is implemented and produces the diagnostics shown in this paper. \emph{Boundary}: The implementation is a working prototype, not a production-grade compiler. Single-device CPU benchmarks are presented in \S5.6, but we do not evaluate large-scale multi-node cluster performance.
  ...
  \item \textbf{Not yet implemented}: Mechanized interactive theorem prover (Coq/Lean) formalization.
\end{itemize}
...
\begin{enumerate}[itemsep=2pt]
  ...
  \item A full compiler implementation comprising ${\sim}29{,}000$ lines across 150 files, 126 passing tests, and 8 standard library modules (\S4).
  ...
  \item A six-pass IR optimization pipeline implementing constant folding, algebraic simplification, common subexpression elimination, dead code elimination, loop invariant code motion, and tensor operation fusion (\S4.5).
  \item A Language Server Protocol (LSP) implementation providing real-time type diagnostics in VS Code with an official extension (\S4.6).
\end{enumerate}
...
\textbf{Rule T-Snapshot-Safe} (safely strips temporal wrapper from past/present data):
\begin{equation}
\frac{\Gamma \vdash e : \ty{Temporal}[T, d] \quad d \leq 0}{\Gamma \vdash e.\ty{snapshot}() : T}
\end{equation}
If $d > 0$ (or direction \ty{future\_to\_past}), calling \ty{.snapshot()} triggers $\textbf{error}[\ty{TemporalLeak}]$, preventing declassification of future data into an untracked raw type.
...
\subsubsection{Integer Offsets and Algebraic Composition}

NEURON supports both coarse binary direction tags (\ty{past\_to\_future} / \ty{future\_to\_past}) and exact \textbf{signed integer offsets}:
\[
\ty{Temporal}[T, \Delta] \quad \text{where } \Delta \in \mathbb{Z}
\]
Under this model, calling \ty{prices.shift(k)} or \ty{prices.lead(k)} produces \ty{Temporal}$[T, \Delta+k]$, while \ty{prices.lag(k)} produces \ty{Temporal}$[T, \Delta-k]$. Offsets compose algebraically at compile time:
\begin{itemize}[leftmargin=1.5em]
  \item Calling \ty{.shift(-5).shift(2)} on \ty{Temporal[T, 0]} evaluates algebraically to $\Delta = -3$ (safe past data).
  \item Binary operations combining two temporal streams $a : \ty{Temporal}[T, \Delta_1]$ and $b : \ty{Temporal}[T, \Delta_2]$ yield a conservative alignment boundary $\Delta_{\text{res}} = \max(\Delta_1, \Delta_2)$. If either operand touches the future ($\Delta > 0$), the entire combination is stamped with the forward dependency.
  \item The safety rule enforces bounded subtyping: when a function expects $\Delta_{\text{req}} \leq 0$, passing any $\Delta_{\text{arg}} \leq 0$ is permitted, while passing $\Delta_{\text{arg}} > 0$ triggers $\textbf{error}[\ty{TemporalLeak}]$ with exact violation diagnostics (reporting the number of leaked forward steps).
  \item For multi-horizon predictive modeling (e.g., forecasting $t+5$ steps ahead), functions returning \ty{Temporal}$[T, +k]$ guarantee compile-time alignment between prediction horizons and loss target timestamps ($t+k \equiv t+k$).
\end{itemize}
...
\subsection{Formal Metatheory and Soundness ($\lambda_{\text{neuron}}$)}
\label{sec:formal-semantics}

To mathematically guarantee that these typing rules prevent bugs in practice, we have formalized the core calculus $\lambda_{\text{neuron}}$ with small-step operational semantics over time-indexed stores $\langle e, \sigma, t \rangle \longrightarrow \langle e', \sigma', t \rangle$. 

The core calculus establishes five metatheoretical guarantees:
\begin{enumerate}[itemsep=2pt]
  \item \textbf{Theorem 1 (Progress)}: If $\emptyset \vdash e : \tau$, then either $e$ is a value $v$ or for any valid store $\sigma$ and evaluation epoch $t$, there exist $e', \sigma'$ such that $\langle e, \sigma, t \rangle \longrightarrow \langle e', \sigma', t \rangle$.
  \item \textbf{Theorem 2 (Subject Reduction / Type Preservation)}: If $\Gamma \vdash e : \tau$ and $\langle e, \sigma, t \rangle \longrightarrow \langle e', \sigma', t \rangle$, then $\Gamma \vdash e' : \tau$.
  \item \textbf{Lemma 1 (Read Horizon Boundedness)}: For any well-typed closed expression with type $\ty{Temporal}[\tau, \Delta]$, the set of absolute timestamps read from the store $\mathcal{R}(e, \sigma, t)$ is strictly bounded by the horizon:
  \[
  \forall t' \in \mathcal{R}(e, \sigma, t).\quad t' \le t + \Delta
  \]
  \item \textbf{Theorem 3 (Temporal Non-Interference --- Proof of Leak-Freedom)}: Let $e$ be a closed expression with $\emptyset \vdash e : \ty{Temporal}[\tau, \Delta]$ where $\Delta \le 0$. If two temporal stores agree on all history up to the present epoch ($\sigma_1 \approx_{\le t} \sigma_2$), then evaluation under both stores terminates with identical values:
  \[
  ...
  \]
  Future mutations, noise, or arrivals ($t' > t$) have zero computational influence on $v_1$. Lookahead leaks are mathematically impossible in well-typed programs.
  \item \textbf{Theorem 4 (Causal Mode Soundness --- Intervention Integrity)}: For any expression $\emptyset \vdash e : \ty{Causal}[\tau, \mathbf{int}]$ under causal model $\mathcal{M}$, evaluation proceeds strictly under the manipulated model $\mathcal{M}_{\overline{X}}$ in which all structural equations for intervened variables $X$ are severed. Subtyping mode isolation prevents interventional terms from combining with observational terms, ensuring observational conditional expectations cannot be silently substituted for interventional queries (automated identification completeness remains an open research direction).
\end{enumerate}

\paragraph{Design Principle: Access Footprint vs. Semantic Dependence.}
NEURON's temporal type system tracks an over-approximation of the temporal access footprint of computation, rather than the extensional semantic dependence of the resulting value on historical observations. An expression such as $s.\ty{eval}() - s.\ty{eval}()$ still bears the temporal horizon of $s$ because evaluation executed a physical store access at that timeline coordinate. Tracking access footprints rather than undecidable semantic equivalence ensures a sound, decidable, and statically verifiable barrier against physical lookahead leaks.
...
The compiler follows a standard multi-pass architecture: source is tokenized by a lexer with INDENT/DEDENT support, parsed by a recursive-descent parser with Pratt precedence for expressions, type-checked in two phases (registration then body checking), lowered to SSA-style IR with basic blocks and terminators, and executed by an interpreter (VM), a JIT compiler (IR $\to$ Rust source $\to$ \texttt{rustc}), an Ahead-Of-Time (AOT) native compiler (\texttt{neuronc aot}), a WebAssembly engine (\texttt{neuron-wasm}), an LSP language server (\texttt{neuronc lsp}), or a PyTorch Python script transpiler. A dedicated GPU backend pass groups contiguous element-wise operations into fused CUDA kernels compiled dynamically via NVRTC, while multi-GPU clusters utilize Ring-AllReduce gradient synchronization primitives (\texttt{distributed.rs}).

\subsection{Advanced Execution Backends \& Tooling Engine}

NEURON features five production-grade compiler backends and developer tooling modules:

\begin{enumerate}[itemsep=2pt]
  \item \textbf{Explicit Precision Engine}: Configurable floating-point precisions (\texttt{f32} and \texttt{f64}) with CLI precision flags (\texttt{--precision f32}, \texttt{--f32}) accelerating single-precision matrix computations.
  \item \textbf{WebAssembly Engine (\texttt{neuron-wasm})}: Lightweight C-ABI WASM library compiled via \texttt{wasm-bindgen} for client-side in-browser evaluation and type checking.
  \item \textbf{Language Server Protocol (LSP) Engine}: Stdio JSON-RPC 2.0 diagnostic server (\texttt{neuronc lsp}) and VS Code extension (\texttt{editors/vscode/}).
  \item \textbf{Ahead-Of-Time (AOT) Native Compiler}: Direct native binary compiler (\texttt{neuronc aot}) with target-specific SIMD vectorization (\texttt{-C target-cpu=native}), executing \textbf{2.08x faster} than the VM interpreter.
  \item \textbf{Multi-GPU Distributed Cluster Engine}: Ring-AllReduce gradient synchronization engine (\texttt{distributed.rs}) managing multi-device CUDA topologies.
\end{enumerate}
...
\texttt{param\_norm\_before} & 1.158016 \\
\texttt{param\_norm\_after} & 0.932155 \\
\texttt{forgotten\_loss\_before} & 0.469637 \\
\texttt{forgotten\_loss\_after} & 0.567157 \\
\texttt{residual\_loss\_retained} & 0.195042 \\
\texttt{bounds\_satisfied} & true \\
\bottomrule
\end{tabular}
\end{center}

The unlearning runtime successfully modifies exactly the 4 weights of tensor \texttt{w}, increases the loss on the patient's data from 0.469637 to 0.567157 (a significant ~21\% shift), and preserves the general model capabilities (residual loss shift of 0.195042 is below the safety threshold, satisfying the bounds).
...
\textbf{Methodology}:
\begin{itemize}[itemsep=2pt]
  \item \textbf{Hardware}: Intel Core i7-1255U (10-core CPU, 1.7 GHz base, 16 GB RAM).
  \item \textbf{Precision}: Double-precision floating-point (\texttt{Float}/\texttt{f64}) across all frameworks to ensure mathematical equivalence.
  \item \textbf{Workloads}:
  \begin{enumerate}[itemsep=2pt]
    \item \emph{MatMul Benchmark}: 200 chained matrix multiplications ($A \times W$) using $256 \times 256$ float64 matrices.
    \item \emph{MLP Training Benchmark}: 100 steps of forward propagation, Mean Squared Error (MSE) loss, backpropagation (gradient tracking), and Adam parameter optimization on a batch size of 64 (Input: 128 $\to$ Hidden: 256 $\to$ Output: 128).
  \end{enumerate}
\end{itemize}

\begin{table}[h]
\centering
\caption{MatMul and MLP Training Execution Times (ms)}
\label{tab:benchmarks}
\begin{tabular}{llrr}
\toprule
\textbf{Workload} & \textbf{Framework} & \textbf{Threads} & \textbf{Time (ms)} \\
\midrule
MatMul (200x) & NEURON VM (Interpreted) & 1 & 9,535.42 \\
& NEURON Native JIT + Parallel DGEMM & Multi & 112.50 \\
& PyTorch CPU (1 Thread) & 1 & 88.24 \\
& Python + NumPy (f64) & Multi & 62.70 \\
& PyTorch CPU (Multi-Thread) & Multi & 65.33 \\
\midrule
MLP Backprop (100 steps) & NEURON VM (Interpreted) & 1 & 351.22 \\
& NEURON Native JIT + Parallel DGEMM & Multi & 148.10 \\
& PyTorch CPU (1 Thread) & 1 & 178.52 \\
& PyTorch CPU (Multi-Thread) & Multi & 98.21 \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{GPU Acceleration (cuBLAS \& Fused CUDA Kernels)}
To evaluate the efficiency of the GPU backend, we measure execution times for chained element-wise operations and matrix multiplication comparing CPU execution with GPU-resident execution on an NVIDIA Tesla T4 GPU (16~GB VRAM, $f64$ precision).

\begin{table}[h]
\centering
\caption{GPU Acceleration Benchmarks (NVIDIA T4, $f64$ precision)}
\begin{tabular}{lrrr}
\toprule
\textbf{Workload / Operator} & \textbf{CPU (ms)} & \textbf{GPU (ms)} & \textbf{Speedup} \\
\midrule
elemwise\_128x128\_chain10 & 2.84 & 0.56 & \textbf{5.1x} \\
elemwise\_256x256\_chain10 & 12.93 & 0.53 & \textbf{24.4x} \\
elemwise\_512x512\_chain10 & 69.82 & 0.55 & \textbf{126.9x} \\
elemwise\_256x256\_chain50 & 308.67 & 2.92 & \textbf{105.7x} \\
matmul\_128x128\_x20 (cuBLAS) & 11.43 & 18.18 & 0.6x \\
matmul\_256x256\_x20 (cuBLAS) & 86.77 & 50.46 & \textbf{1.7x} \\
\bottomrule
\end{tabular}
\end{table}

NEURON's GPU backend achieves up to \textbf{164.9x speedup} on large-scale element-wise operations through operator fusion (compiling chained operations into a single GPU kernel via NVRTC to minimize VRAM bandwidth bounds). The cuBLAS integration delivers a \textbf{1.3x speedup} on $256 \times 256$ matrix multiplication. Multi-device distributed GPU cluster scaling remains future work.
...
\textbf{Causal Inference Libraries.} Our causal model representation is related to DoWhy~\cite{sharma2020} and EconML~\cite{battocchi2019}, which implement causal algorithms in Python without type-level enforcement.

\textbf{Gradual Typing \& Effect Systems.} Gated uncertainty warning systems share theoretical roots with gradual typing systems like Pyret~\cite{pyret2016}, which combine static check boundaries with runtime flexibility. Our effect system, which isolates mutating states and random state effects in machine learning, draws design principles from language research in algebraic effects and handlers like Hazel~\cite{hazel2018} (which utilizes type-level effects and holes for interactive execution) and Rholang~\cite{rholang2017} (enforcing concurrent behavioral contracts). NEURON differs by specializing these abstractions for numerical safety, specifically separating pure forward model evaluation from parameter optimization and state perturbation effects.
...
\textbf{What this system does not do.} NEURON enforces consistency of causal reasoning within a declared model; it does not verify that the declared model is correct. If a developer declares an incorrect causal graph, the type checker will enforce that wrong structure. Similarly, the type system does not prove that temporal annotations are accurate, nor does it guarantee uncertainty bounds are calibrated. We have presented single-device benchmarks comparing NEURON against reference CPU implementations, but it has not been benchmarked on large-scale distributed clusters.
...
\textbf{Future work.} Multi-device and distributed GPU execution, and interactive mechanized proof verification in Coq or Lean.
...
\bibitem{pyret2016}
S.~Krishnamurthi et al., ``Pyret: A programming language designed for education,'' 2016. Available: \url{https://www.pyret.org/}.

\bibitem{hazel2018}
C.~A.~Omar et al., ``Live functional programming with typed holes,'' in \emph{POPL}, 2018.

\bibitem{rholang2017}
L.~G.~Meredith, ``Rholang specification,'' CoRR abs/1709.07635, 2017.
